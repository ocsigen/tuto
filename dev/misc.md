...
Using `val
Eliom.Content.Html.Id.create_global_elt` we can create an xml node manipulated 'by reference'. If a reference to a script node is included in a page, it will only be loaded the first time it appears in the header. This is the case for `oclosure_script` above.
