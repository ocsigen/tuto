...
{client{ ... }}
```
, with the difference that what was previously in the client value `init_client` is now in the function `launch_client_canvas`.
...
```
`Lwt.cancel t` stops thread t. In this case it also closes the bus on which t is listening. For more informations see the [Lwt programming guide](https://ocsigen.org/lwt/latest/) and `Eliom.Bus`.
...
let launch_client_canvas bus image_elt canvas_elt slider =
  ...
  (* The color palette: *)
  let colorpicker, cp_sig = Ot.Color_picker.make () in
  ...
  let line ev =
    ...
    Lwt.return ()
  in
  let t = Lwt_stream.iter (draw ctx) (Eliom.Bus.stream bus) in
  let drawing_thread =
...
let launch_server_canvas () =
  let bus = Eliom.Bus.create [%json: messages] in
  ...
  in
  let _ = Lwt_stream.iter draw_server (Eliom.Bus.stream bus) in
  bus,image_string

let graffiti_info = Hashtbl.create 0

let imageservice =
  Eliom.Registration.String.create
    ~path:(Eliom.Service.Path ["image"])
    ~headers:
       (Cohttp.Header.add_list (Cohttp.Header.init ())
          [(Ocsigen_http.Header.Name.(to_string cache_control), "no-cache") ;
           (Ocsigen_http.Header.Name.(to_string expires), string_of_int 0)])
    ~meth:
      (Eliom.Service.Get
         (let open Eliom.Parameter in string "name" ** int "q"))
	...
	Lwt.return (image_string (), "image/png")
      with
	| Not_found -> Lwt.fail Eliom.Common.Eliom_404)
...
let multigraffiti_service =
  Eliom.Service.create
    ~path:(Eliom.Service.Path [""])
    ~meth:(Eliom.Service.Get (Eliom.Parameter.(suffix (string "name"))))
    ()
...
Using `val
Eliom.Content.Html.Id.create_global_elt` we can create an xml node manipulated 'by reference'. If a reference to a script node is included in a page, it will only be loaded the first time it appears in the header. This is the case for `oclosure_script` above. <!--wodoc:end-->
...
let start_drawing name image canvas slider =
  ...
  ignore [%client
    (let canceller =
     ...
     in
     Eliom.Client.onunload (fun () -> stop_drawing canceller; None)
     : unit)
  ]
```
The function registered by `Eliom.Service.onunload` will be called when the page change inside the application.
...
```ocaml
let connection_service = Eliom.Service.create
    ~path:Eliom.Service.No_path
    ~meth:(Eliom.Service.Post (
      Eliom.Parameter.unit,
      Eliom.Parameter.(string "name" ** string "password")
    ))
    ()

let disconnection_service = Eliom.Service.create
    ~path:Eliom.Service.No_path
...
let () = Eliom.Registration.Action.register
...
let () = Eliom.Registration.Action.register
  ...
  (fun () (name, password) ->
    match%lwt check_pwd name password with
      | true -> Eliom.State.set_volatile_data_session_group
	~scope:Eliom.Common.default_session_scope name;
	Lwt.return ()
      | false -> Lwt.return ())
...
let () =
  Eliom.Registration.Action.register
    ...
    (fun () () ->
      Eliom.State.discard ~scope:Eliom.Common.default_session_scope ())
...
struct
  ...
  let translate page =
    match Eliom.State.get_volatile_data_session_group
      ~scope:Eliom.Common.default_session_scope () with
	| None -> default_content ()
	| Some username -> page username
end

module Connected =
  Eliom.Registration.Customize (My_app) (Connected_translate)
```
