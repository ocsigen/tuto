...
let%server _ = Eliom.Content.Html.D.(
  Ex_app.create
    ~path:(Eliom.Service.Path [""])
    ~meth:(Eliom.Service.Get Eliom.Parameter.unit)
    (fun () () ->
       Lwt.return
         (Eliom.Tools.D.html ~title:"tutowidgets" ~css:[["css"; "ex.css"]]
...
let%client mywidget s1 s2 = Eliom.Content.Html.D.(
...
let%client _ =
  ...
  Js_of_ocaml.Dom.appendChild
    ...
    (Eliom.Content.Html.To_dom.of_element (mywidget "Click me" "Hello!"));
  Lwt.return ()
```
...
```
The name of the project must match the name given to the functor `Eliom.Registration.App`.
...
- The `##` is used to call a JS method from OCaml and `##.` to access a JS object field (See Js\_of\_ocaml's documentation: `Ppx_js`).
...
let%client switch_visibility elt =
  let elt = Eliom.Content.Html.To_dom.of_element elt in
...
let%client mywidget s1 s2 = Eliom.Content.Html.D.(
  ...
  let content = div ~a:[a_class ["content"]] [txt s2] in
  Lwt.async (fun () ->
    Js_of_ocaml_lwt.Lwt_js_events.clicks (Eliom.Content.Html.To_dom.of_element button)
...
- `Eliom.Content.Html.To_dom.of_element`, `Js_of_ocaml.Js.string` and `Js_of_ocaml.Js.to_bool` are conversion functions between OCaml values and JS values.
...
<!--wodoc:@ class=server-->
```ocaml
let%server mywidget s1 s2 = Eliom.Content.Html.D.(
  let button  = div ~a:[a_class ["button"]] [txt s1] in
  let content = div ~a:[a_class ["content"]] [txt s2] in
  let _ = [%client
    (Lwt.async (fun () ->
...
let%server _ = Eliom.Content.Html.D.(
  Ex_app.create
    ...
    (fun () () ->
       Lwt.return
         (Eliom.Tools.D.html ~title:"ex" ~css:[["css"; "ex.css"]]
            (body [h2 [txt "Welcome to Ocsigen!"];
...
<!--wodoc:@ class=shared-->
```ocaml
let%shared mywidget s1 s2 = Eliom.Content.Html.D.(
  let button  = div ~a:[a_class ["button"]] [txt s1] in
  let content = div ~a:[a_class ["content"]] [txt s2] in
  let _ = [%client
    (Lwt.async (fun () ->
       Js_of_ocaml_lwt.Lwt_js_events.clicks (Eliom.Content.Html.To_dom.of_element ~%button) (fun _ _ ->
	 !close_last ();
...
let%server _ = Eliom.Content.Html.D.(
  Ex_app.create
	...
	[%client
	    (Js_of_ocaml.Dom.appendChild
	       (Js_of_ocaml.Dom_html.document##.body)
               (Eliom.Content.Html.To_dom.of_element (mywidget "Click me" "client side"))
               : unit)
	] in
      Lwt.return
        (Eliom.Tools.D.html ~title:"ex" ~css:[["css"; "ex.css"]]
...
<!--wodoc:@ class=shared-->
```ocaml
let%shared mywidget set s1 s2 = Eliom.Content.Html.D.(
  let button  = div ~a:[a_class ["button"]] [txt s1] in
...
let%server _ = Eliom.Content.Html.D.(
  Ex_app.create
	...
	Js_of_ocaml.Dom.appendChild
	  (Js_of_ocaml.Dom_html.document##.body)
	  (Eliom.Content.Html.To_dom.of_element (mywidget ~%set2 "Click me" "client side"))
	  : unit)
	      ] in
      Lwt.return
        (Eliom.Tools.D.html ~title:"ex" ~css:[["css"; "ex.css"]]
