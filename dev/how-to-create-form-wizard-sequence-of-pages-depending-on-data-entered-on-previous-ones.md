...
let calc = Eliom.Service.create
    ~path:(Eliom.Service.Path ["calc"])
...
let calc_i_handler i () =
  ...
  let calc_result =
    Eliom.Registration.Html.attach_get
      ~scope:Eliom.Common.default_session_scope
      ~fallback:calc
      ~get_params:(Eliom.Parameter.int "j")
      ~timeout:120.
      (fun j () ->
...
let () =
  Eliom.Registration.Html.register calc   calc_handler;
  Eliom.Registration.Html.register calc_i calc_i_handler
```